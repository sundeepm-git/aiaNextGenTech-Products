...
"""
    ...
    def generate_variables_tf(self):
            ...
            for key, value in d.items():
                ...
                if is_sensitive(key):
                    ...
                    variables[var_name] = {
                        ...
                        'sensitive': True,
                        'default': None  # Sensitive fields should not have defaults
                    }
                            ...
                            if m:
                                ...
                                if not is_data_sourced(jval) and not is_id_field(jkey) and not is_sensitive(jkey):
                                    ...
                                    variables[var_name] = {
                                        ...
                                        'default': jval.strip('"') if isinstance(jval, str) else jval
                                    }
                                    resource_groups.setdefault(group_label, []).append(var_name)
                        continue
        ...
        for var_name, var_info in variables.items():
            descriptions[var_name] = var_info.get('desc', f'Variable for {var_name}')
            types[var_name] = var_info.get('type', 'string')
            if 'default' in var_info and var_info['default'] is not None:
                defaults[var_name] = var_info['default']
            if var_info.get('sensitive', False):
        ...
        for var_name in variables:
            ...
            if var_name in defaults and var_name not in sensitive:
                default_val = defaults[var_name]
                if type_val == 'map(string)' and isinstance(default_val, dict):
                    tags_lines = ['  default = {']
                    ...
                    if isinstance(default_val, str):
                        block += f'\n  default = {default_val.strip(chr(34)).lower()}'
                    else:
                        block += f'\n  default = {str(default_val).lower()}'
                elif type_val == 'number':
                    if isinstance(default_val, str):
                        block += f'\n  default = {default_val.strip(chr(34))}'
                    else:
                        block += f'\n  default = {default_val}'
                elif type_val.startswith('list'):
                    if isinstance(default_val, str) and default_val.startswith('[') and default_val.endswith(']'):
                        ...
                        hcl_list = '[' + ', '.join(f'"{item}"' for item in default_val) + ']'
                        block += f'\n  default = {hcl_list}'
                    else:
                        block += f'\n  default = {default_val}'
                else:
                    # Strip surrounding HCL quotes to avoid double-quoting
